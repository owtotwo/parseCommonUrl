...
(function() {
    ...
    function parseCommonUrl(url) {
        ...
        // parse url, host: a.b.example.co.uk
        const [ _url, scheme, userInfo, host, port, path, query, fragment ] = regexResult;
        ...
        const rootDomain = [ siteName, TLD ].join('.');

        return { 
            scheme: scheme, protocol: scheme, userInfo: userInfo, host: host, domain: host, 
            subDomain: subDomain, siteName: siteName, TLD: TLD, rootDomain: rootDomain, 
    ...
    try {
        ...
        if (!e.message.startsWith('module')) throw e;
    }

    // AMD (requirejs)
    try {
        requirejs && define(function () { return parseCommonUrl; });
    } catch (e) {
        // define is not defined
        if (!e.message.startsWith('requirejs')) throw e;
    }

    // CMD (seajs)
    try {
        seajs && define(function(_require, _exports, module) {
            module.exports = parseCommonUrl;
        });
    } catch (e) {
        // define is not defined
        if (!e.message.startsWith('seajs')) throw e;
    }
    
    // Browser
    try {
        if (window.parseCommonUrl === undefined) window.parseCommonUrl = parseCommonUrl;
    } catch (e) {
        // window is not defined
        if (!e.message.startsWith('window')) throw e;
    }
